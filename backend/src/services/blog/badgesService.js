...
async function assignBadges(userId) {
  ...
  postPerUser.forEach((post) => {
    if (
      post.likes >= 2 &&
      !user.badges.some(
        (badge) => badge._id.toString() === garden_guru._id.toString()
      )
    ) {
      user.badges.push(garden_guru);
      response.name = garden_guru.name;
      response.image = garden_guru.image;
      response.description = garden_guru.description;
    }
    if (
      post.likes >= 4 &&
      !user.badges.some(
        (badge) => badge._id.toString() === green_thumb._id.toString()
      )
    ) {
      user.badges.push(green_thumb);
      response.name = green_thumb.name;
      response.image = green_thumb.image;
      response.description = green_thumb.description;
    }
    if (
      post.comments.length >= 6 &&
      !user.badges.some(
        (badge) => badge._id.toString() === discussion_dynamo._id.toString()
      )
    ) {
      user.badges.push(discussion_dynamo);
      response.name = discussion_dynamo.name;
      response.image = discussion_dynamo.image;
      response.description = discussion_dynamo.description;
    }
    if (
      postPerUser.length >= 2 &&
      !user.badges.some(
        (badge) => badge._id.toString() === seed_sower._id.toString()
      )
    ) {
      user.badges.push(seed_sower);
      response.name = seed_sower.name;
      response.image = seed_sower.image;
      response.description = seed_sower.description;
    }
    if (
      postPerUser.length >= 5 &&
      !user.badges.some(
        (badge) => badge._id.toString() === harvest_master._id.toString()
      )
    ) {
      user.badges.push(harvest_master);
      response.name = harvest_master.name;
      response.image = harvest_master.image;
      response.description = harvest_master.description;
    }
  });
